...
async function createUniquenessConstraints(session) {
  await session.run('CREATE CONSTRAINT IF NOT EXISTS FOR (r:Root) REQUIRE r.id IS UNIQUE');
  ...
  await session.run('CREATE CONSTRAINT IF NOT EXISTS FOR (rc:ReactComponent) REQUIRE rc.id IS UNIQUE');
}
...
async function analyzeFile(filePath, session, rootName, baseUrl, projectRoot, rootId) {
  ...
  function mergeNodeQuery(labels, id, properties, onCreateExtra = '', onMatchExtra = '') {
    const labelString = Array.isArray(labels) ? labels.join(':') : labels;
    let setClause = Object.keys(properties).map(key => `n.${key} = $${key}`).join(', ');
  ...
  function ensureClassNode(classId, className, filePath, line) {
    if (!processedNodes.has(classId)) {
      ...
      queries.push(mergeNodeQuery('Class', classId, {
        name: className,
        project: rootName,
        file: filePath,
  ...
  function ensureFunctionNode(funcId, funcName, filePath, line) {
    if (!processedNodes.has(funcId)) {
      ...
      queries.push(mergeNodeQuery('Function', funcId, {
        name: funcName,
        project: rootName,
        file: filePath,
  ...
  function ensureMethodNode(methodId, methodName, filePath, line) {
    if (!processedNodes.has(methodId)) {
      ...
      queries.push(mergeNodeQuery('Method', methodId, {
        name: methodName,
        project: rootName,
        file: filePath,
  ...
  function ensureReactComponentNode(componentId, componentName, filePath, line) {
    if (!processedNodes.has(componentId)) {
      processedNodes.add(componentId);
      queries.push(mergeNodeQuery('ReactComponent', componentId, {
        name: componentName,
        project: rootName,
        file: filePath,
  ...
  function ensureCalleeNode(calleeId, calleeName) {
    ...
    if (!processedNodes.has(calleeId)) {
      ...
      queries.push(mergeNodeQuery('Function', calleeId, {
        name: calleeName,
        project: rootName,
      }));
  ...
  traverse(ast, {
    ...
    ClassDeclaration(path) {
      ...
      node.body.body.forEach(classMember => {
        if (classMember.type === 'ClassMethod' && classMember.key.type === 'Identifier') {
    ...
    ClassMethod(path) {
      ...
      const { node } = path;
      if (!path.parentPath.parent || path.parentPath.parent.type !== 'ClassDeclaration') return;
    ...
    CallExpression(path) {
      ...
      if (callee.type === 'Identifier') {
        ...
        const objectName = callee.object && callee.object.type === 'Identifier' ? callee.object.name : 'unknownObj';
        const propertyName = callee.property && callee.property.type === 'Identifier' ? callee.property.name : 'unknownProp';
      ...
      if (callerFunc && callerFunc.node) {
        if (callerFunc.node.type === 'FunctionDeclaration' && callerFunc.node.id) {
...
async function analyzeDirectory(dir, session, rootName, baseUrl, projectRoot, rootId) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      await analyzeDirectory(filePath, session, rootName, baseUrl, projectRoot, rootId);
    } else if (file.endsWith('.js') || file.endsWith('.jsx') || file.endsWith('.ts') || file.endsWith('.tsx')) {
      await analyzeFile(filePath, session, rootName, baseUrl, projectRoot, rootId);
    }
  }
}
...
async function main() {
  ...
  try {
    ...
    await analyzeDirectory(projectRoot, session, rootName, baseUrl, projectRoot, rootId);
  } catch (error) {
